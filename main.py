...
app = FastAPI(title="vet.ector Veterinary Alert API", version="1.2.0-wahis-csv-import")
...
def upsert_veterinarian(row: Dict[str, Any]) -> str:
    ...
    with connect() as conn:
        ...
        """, (external_id,row.get("name"),row.get("type","Veterinario"),row.get("availability",""),row.get("phone",""),float(row.get("lat")),float(row.get("lon")),row.get("city",""),row.get("region",""),json.dumps(row.get("services",[]),ensure_ascii=False)))
        conn.commit()
    return "updated" if existing else "inserted"
...
def sync_official_events() -> Dict[str, Any]:
    return _sync_connector(OfficialDemoConnector(), "OFFICIAL_DEMO")


def sync_wahis_events() -> Dict[str, Any]:
    return _sync_connector(WahisCsvConnector(), "WAHIS_CSV")


def _sync_connector(connector, default_source: str) -> Dict[str, Any]:
    started_at=now_iso(); inserted=updated=received=skipped=0
    try:
        raw_events=connector.fetch(); received=len(raw_events)
        ...
        log_sync(connector.source_name,"success",f"{connector.source_name} sync completed; skipped={skipped}",received,inserted,updated,started_at)
        return {"status":"success","source":connector.source_name,"received":received,"inserted":inserted,"updated":updated,"skipped":skipped}
    except Exception as exc:
        log_sync(connector.source_name,"error",str(exc),received,inserted,updated,started_at); raise


def haversine_km(lat1: float, lon1: float, lat2: float, lon2: float) -> float:
    dlat=math.radians(lat2-lat1); dlon=math.radians(lon2-lon1); rlat1=math.radians(lat1); rlat2=math.radians(lat2)
...
def matches_animal_filter(row: Dict[str, Any], animal_filter: str) -> bool:
    ...
    filters={"companion":["dog","cat","cane","gatto","companion"],"dogs":["dog","cane"],"cats":["cat","gatto"],"farm":["bovine","bovino","swine","suino","poultry","avicoli","ovine","ovino"],"bovine":["bovine","bovino","cattle"],"swine":["swine","suino","cinghiale","pig"],"poultry":["poultry","avicoli","volatile","avian"],"equine":["equine","equini","horse","cavallo"]}
...
def get_wahis_status() -> Dict[str, Any]:
    with connect() as conn:
        row=conn.execute("SELECT * FROM sync_log WHERE source = 'WAHIS_CSV' ORDER BY id DESC LIMIT 1").fetchone()
